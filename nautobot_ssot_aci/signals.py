...
def aci_create_custom_field_pod(apps, **kwargs):
    """Add custom field NodeId."""
    cf_model = apps.get_model("extras", "CustomField")
    logger.info("Creating Custom Field: pod-id")
    cf_model.objects.update_or_create(
        name="pod-id",
        label="Pod Id",
        description="Pod ID pulled from ACI fabric by SSoT plugin",
        filter_logic="loose",
        weight=100,
        type="integer",
    )
    ct = apps.get_model("contenttypes", "ContentType")
    ct_obj = ct.objects.get(model="device")
    ct_obj.custom_fields.set(objs=[cf_model.objects.get(name="pod-id")])
    ct_obj.save()